...
class PoseGenerator(nn.Module):
    def __init__(self, args, pose_dim, n_words, word_embed_size, word_embeddings, z_obj=None):
        ...
        if self.input_context == 'both':
            ...
            self.in_size = 32 + pose_dim + 1  # audio or text only
            # self.in_size = 32 + 32 + pose_dim + 1  # audio or text only
    ...
    def forward(self, pre_seq, in_text, in_audio, vid_indices=None):
        ...
        # print(pre_seq.shape) # [128, 34, 28]
        # print(in_text.shape) # [128, 34]
        # print(in_audio.shape) # [128, 36264]
        # print(vid_indices.shape) # [128]
        ...
        if self.input_context != 'none':
            # audio
            audio_feat_seq = self.audio_encoder(in_audio)  # output (bs, n_frames, feat_size)  [128,34,32]
        ...
        if self.z_obj:
            if self.speaker_embedding:
                ...
                z_context = model.embedding_net.reparameterize(z_mu, z_logvar)  # [128,16]
            else:
                z_mu = z_logvar = None
        ...
        if self.input_context == 'both':
            in_data = torch.cat((pre_seq, audio_feat_seq, text_feat_seq), dim=2) # [128, 34, 92=28+32+32]
        ...
        if z_context is not None:
            ...
            in_data = torch.cat((in_data, repeated_z), dim=2)  # [128, 34, 108=92+16]

        output, decoder_hidden = self.gru(in_data, decoder_hidden)
        output = output[:, :, :self.hidden_size] + output[:, :, self.hidden_size:]  # sum bidirectional outputs
        output = self.out(output.reshape(-1, output.shape[2]))
        decoder_outputs = output.reshape(in_data.shape[0], in_data.shape[1], -1)  # [128, 34, 27]
